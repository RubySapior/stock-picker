...
def _update_bench_closes(today):
    ...
    Hosting fix: uses store.read_json_locked so 1000 concurrent leaderboard
    builds don't race with update.py's daily atomic write of ohlc_cache.json.
    """
    try:
        cache = store.read_json_locked(os.path.join(BASE, "ohlc_cache.json"), {}) or store.read_json(os.path.join(BASE, "ohlc_cache.json"), {})
...
def build_leaderboards(data=None):
    ...
    cache = store.read_json_locked(BENCH_CACHE, {}) or store.read_json(BENCH_CACHE, {}) or {}
...
def write_leaderboards(payload=None):
    ...
    # Hosting fix: atomic write via store.write_text_atomic (tmp+rename under
    # per-path lock) so concurrent readers never see a half-written JS file
    # when 1000 users trigger a leaderboard rebuild at once.
    text = "/* AUTO-GENERATED by leaderboards.py from portfolio.json / community/ - do not edit by hand. */\n"
    text += "window.LEADERBOARDS = "
    text += json.dumps(payload, indent=2, ensure_ascii=False)
    text += ";\n"
    store.write_text_atomic(LEADERBOARD, text)
    return payload
