...
| `risk.py` | Risk-parity sizing engine, Phase 1 SHADOW (site 0.5.6.17): product-level EMA21 ATR% targets, book heat, cap headroom, deadband vs `meta.ai_state.acted_conviction`, atomic rotation pre-flight. `shadow_pass()` is strictly read-only, logs to `logs/shadow_execution.log` (gitignored, unserved). Live behavior unchanged. | **YES** — engine layer; shadow only until Phase 2 |
